...
public class BanCommandConsole {
	...
	public boolean runBanCommand(CommandSender sender, String[] args) {
		...
		// check if there is a uuid stored for the particular player
		if (uuid == null) {
		...
		if (target != null) {
			...
			target.kickPlayer(Lang.BAN_FORMAT.parseBanFormat((new Main()).getLatestName(uuid), banMessage));
								
		}
		
		sender.sendMessage(Lang.BAN_SUCCESS.parseName((new Main()).getLatestName(uuid)));
		
		Banana.getDatabaseManager().logCommand(CommandType.BAN, null, args, true);
		
		// check if announcements are enabled for this command.. then release the annoucnement
		if (Values.ANNOUNCE_BAN) {
			
			Action.broadcastMessage(Action.BAN, Lang.BAN_BROADCAST.parseWarningBroadcast(BannerType.CONSOLE.toString(), (new Main()).getLatestName(uuid), banMessage));
