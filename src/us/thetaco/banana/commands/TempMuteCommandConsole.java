...
public class TempMuteCommandConsole {

	public boolean runTempMuteCommand(CommandSender sender, String[] args) {
		
		if (args.length < 2) {
			sender.sendMessage(Lang.TEMPMUTE_WRONG_ARGS.toString());
			return true;
		}
		...
		if (target == null) {
			...
			if (target.hasPermission("banana.immune.tempmuted")) {
				sender.sendMessage(Lang.CANNOT_BE_TEMP_MUTED.toString());
				return true;
			}
		...
		if (Banana.getMuteCache().isMuted(uuid)) {
			...
			if (Banana.getMuteCache().isTempMuted(uuid)) {
				sender.sendMessage(Lang.ALREADY_TEMP_BANNED.toString());
			} else {
				sender.sendMessage(Lang.ALREADY_MUTED.toString());
			}
		...
		sender.sendMessage(Lang.TEMPMUTE_SUCCESS.parseName((new Main()).getLatestName(uuid)));
		
		Banana.getDatabaseManager().logCommand(CommandType.TEMP_MUTE, null, args, true);
		
		// check if announcements are enabled for this command.. then release the annoucnement
		if (Values.ANNOUNCE_TEMPMUTE) {
												
			Action.broadcastMessage(Action.TEMPMUTE, Lang.TEMP_MUTE_BROADCAST.parseTimeBroadcast(days, hours, minutes, seconds, (new Main()).getLatestName(uuid), Lang.CONSOLE_NAME.toString(), message));
